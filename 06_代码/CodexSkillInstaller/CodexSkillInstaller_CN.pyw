# -*- coding: utf-8 -*-
"""
Codex Skill 安装器 + FFmpeg 修复工具

功能：
1. 选择一个 Skill Markdown 文件，自动注册到 Codex。
...
def install_skill(md_path):
    ...
    target_dir = Path.home() / ".codex" / "skills" / skill_name
    target_dir.mkdir(parents=True, exist_ok=True)
    target_file = target_dir / "SKILL.md"
    shutil.copy2(path, target_file)

    skill_name_var.set(skill_name)
    target_path_var.set(str(target_file))
    status_var.set("已安装 Skill：" + skill_name)
...
def open_skills_folder():
    folder = Path.home() / ".codex" / "skills"
    folder.mkdir(parents=True, exist_ok=True)
    try:
        os.startfile(str(folder))
    except Exception:
        messagebox.showinfo("Codex Skills 目录", str(folder))
...
def safe_extract_zip(zip_file, target_dir):
    ...
    for member in zip_file.infolist():
        ...
        if not str(member_path).startswith(str(target_dir)):
            raise RuntimeError("压缩包包含不安全路径，已停止解压。")
...
def build_ui():
    ...
    tk.Label(
        root,
        text="一键安装 Skill，并为视频类 Skill 准备 FFmpeg",
        bg="#f7f7f5",
        fg="#666666",
    ...
    tk.Label(info, textvariable=target_path_var, bg="#f7f7f5", fg="#222222", font=("Consolas", 8), wraplength=400, justify="left").grid(row=1, column=1, sticky="w", pady=(5, 0))
