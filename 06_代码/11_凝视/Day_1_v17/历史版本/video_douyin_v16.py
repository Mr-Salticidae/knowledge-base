...
- 完全去掉钟表咔嗒音效(BGM 自身已有 mechanical clockwork ticks)
- 保留色差转场(全图通用,Veritia 强化)
- Morgana 和 Veritia 因时长太短不显示英文名字幕

时间线(用户卡点):
0.000 - 3.000  开场字幕
3.000 - 5.900  01 SCARLATTA  (2.90s)
5.900 - 8.800  02 VESPER     (2.90s)
8.800 - 11.300 03 NULL-07    (2.50s)
11.300-11.900  04 MORGANA    (0.60s — 快闪,无字幕)
11.900-13.300  05 SELENWE    (1.40s)
13.300-14.000  06 VERITIA    (0.70s — 强化色差,无字幕)
...
SEG = {
    'opening':   (0.000, 3.000),    # 3.00s
    'scarlatta': (3.000, 2.900),    # 2.90s
    'vesper':    (5.900, 2.900),    # 2.90s
    'null07':    (8.800, 2.500),    # 2.50s
    'morgana':   (11.300, 0.600),   # 0.60s 快闪(无字幕)
    'selenwe':   (11.900, 1.400),   # 1.40s
    'veritia':   (13.300, 0.700),   # 0.70s 强化色差(无字幕)
    'ending':    (14.000, 1.000),   # 1.00s
}

# 哪些角色显示英文名字幕(时长太短的不显示)
SHOW_NAME = {
    'scarlatta': True,
    'vesper':    True,
    'null07':    True,
    'morgana':   False,  # 0.6s 太短
    'selenwe':   True,
    'veritia':   False,  # 0.7s 太短(且压轴需要纯视觉冲击)
}
...
def build_video():
    ...
    for key in char_keys:
        char = next(c for c in CHARACTERS if c.num == char_num_map[key])
        duration = SEG[key][1]
        show_name = SHOW_NAME[key]
...
def export_no_bgm_version(video):
    """抖音版:无 BGM,无音效(完全静音视频,等抖音内加 BGM)"""
    print("\n========== 导出抖音版(无 BGM,完全静音) ==========")
    # 不加任何音轨,纯视频
...
def main():
    print("="*60)
    print("《凝视》v16 - 用户卡点 + 去除钟表音效")
    print("="*60)
    
    print("\n时间线:")
    print(f"{'段':<12} {'起':<8} {'止':<8} {'时长':<8} 备注")
    print('-' * 60)
    for key, (start, dur) in SEG.items():
        end = start + dur
        note = ""
        if key in SHOW_NAME:
            note = "(无字幕)" if not SHOW_NAME[key] else ""
        print(f"  {key:<10} {start:>5.3f}s {end:>5.3f}s {dur:>5.3f}s   {note}")
