...
> ⬆️ **本节已升格独立档：[[标签相同不等于行为相同_第三方代跑平台律_v1]]**（2026-08-07）。升格由第二例触发：liblib.tv 内置 GPT 出的"透明背景"是**把灰白棋盘格画进像素**的假透明，换 GPT 官网即得真 alpha。那一例比悠船更硬——悠船是**降质**（能出，差一档），liblib 的 GPT 是**能力缺失 + 伪造**（给不了，还交出骗过验收的替代品）。判据与验收动作看新档。
...
- ~~验证其他第三方 MJ 平台（liblib MJ V7 等）后，§10.5"标签相同 ≠ 行为相同" 升格为独立元方法论~~ → **已完成（2026-08-07）**：由 liblib.tv 的 GPT Image 假透明一例触发（跨工具族，非 MJ），升格为 [[标签相同不等于行为相同_第三方代跑平台律_v1]]
