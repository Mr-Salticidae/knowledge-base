...
## 风格层反默认验证 🆕 v1.1（2026-07-10 实测）

v1 只实测了**人物层**默认偏好（甜化/幼化）。学员答疑案例补上了**风格层**验证：niji 7 的现代高精度二次元先验（锐利线条/高光泽/高饱和）会压制复古手绘风——单写 `Studio Ghibli style` 一个词必被带跑；但**反默认配方可以压住**：

- 正向锚：`1980s Ghibli anime film still, retro cel animation, hand-painted watercolor background, flat cel shading, soft muted earthy colors, subtle film grain`
- 参数：`--raw --stylize 50`
- 实测结果：**4/4 全项命中**（平涂线条/水彩背景/低饱和土色/柔和夕照），复古赛璐璐质感完整出片
- **消融对比**：去掉 `--no glossy, modern digital anime...` 段，风格层同样全中——风格层的决定性武器是**正向锚+低 stylize**，`--no` 独立贡献不显著（单次对比；`--no` 对道具稳定性的作用待多 roll 验证）

这同时给武器 7 的待验证规律（`--raw` 反默认更直接）添了一个正面数据点。完整案例见 [[动作错做减法_风格错辨顺逆_学员答疑案例_v1]]。

**结论口径**：niji 7 的默认偏好在人物层和风格层都强势,但都可用"重复+具象+反向"压制;复古手绘需求下 niji 6 是顺默认省力路径,niji 7 是逆默认可行路径(成本更高、质感可控)。
...
- 下游案例：[[动作错做减法_风格错辨顺逆_学员答疑案例_v1]] · 学员答疑证实 niji 7 在**风格层**同样默认强势且可被反默认配方压制——已回填 §风格层反默认验证
...
- **v1.1 - 2026-07-10 - 风格层反默认验证回填**（学员答疑案例：复古手绘/吉卜力风被现代二次元先验压制,反默认配方 4/4 压住;含去 --no 消融;--raw 待验证规律 +1 正面数据点）

升级触发：
- 多次 niji 7 项目反向验证后做 v2（人物层+风格层已各一场,第三场后升）
