...
| creator-weekly-report | v1.0 | 2026-06-29 | 创作者周报:一周活动(git+复盘+skill+发布)→ 事实先行结构化周报(取数→冻结→写→归档→可选对外);日频复盘之上的周聚合层 | Claude | 首期实例 [[2026-W26_创作周报]] 实跑自验证 |
...
### creator-weekly-report v1.0（2026-06-29，从「日频复盘缺周聚合层」缺口封装）

来源：本会话发现知识库是日频复盘 + 持续沉淀的节奏，但缺一层把一周横向收拢的视角——「周报」一直在做却没沉淀成 skill。本次把它封装为可复用工作流，并用上周（2026-W26）真实 git 活动实跑出首期周报实例自验证。

- [[creator-weekly-report/SKILL.md]] — Claude
- 首期实例：[[2026-W26_创作周报]]（`00_仓库维护/周报/`）
- 周报序列入口：[[周报索引]]

定位：日频复盘之上的「周」聚合层。五阶段——取数（自动抓 git 提交轨迹、改动落区、新增 skill / 方法论；库外发布物与平台数据明确标缺口让作者补）→ 事实冻结（借 `aigc-postmortem` 顶部不可改事实区）→ 写周报（本周一句话 / 亮点 / 方法论沉淀带验证等级 / 踩坑 / 跨周连续性 / 下周计划）→ 归档自用内核版到 `00_仓库维护/周报/` → 可选对外版到 `08_对外分发/`（借 `knowledge-base-curator` 内核/对外双产出）。核心是横向聚合与跨周趋势（铁律养成轨迹、计划回填、主线漂移检测），不替代单作品深挖。
