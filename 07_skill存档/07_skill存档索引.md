...
### insight-public-post v1.0（2026-07-05，2 轮评测封装）

来源：跳蛛先生加入 B 站「AI 开发者小站」后，确立「知识库每沉淀一篇 insight 就同步产出一篇面向所有用户的公开版对外帖」这一固定惯例，将其封装为可复用 skill。

- [[insight-public-post_v1.0_SKILL.md]] — Claude

定位：把知识库内核档（含双链、面向自己）改写成陌生人可冷读的公开帖，交付到「可整篇粘贴发小站」为止（不替用户发帖）。三步——① 定位内核档 + 读黄金样例；② 按 08 分发体例（零双链/名词小抄/全角标点）+ B 站人设（整活自黑/事故现场钩子/结尾可抄清单）+ 三层脱敏改写；③ 归档 08_对外分发 + 索引登记 + 「分发:」前缀提交推送。

封装过程（skill-creator 2 轮迭代，with_skill vs 裸跑对照）暴露并固化的关键点：
- **skill 的最稳定增量是「三层闭环意识」**——归档/索引登记/推送三步裸跑完全不会自发补上；
- **脱敏是核心价值**——裸跑会把真实 base 路径 `/above-the-web` 原样写进帖子（可反查到仓库），故 v1.0 加硬规则「内核档真实标识符发布前搜一遍换示例值」；
- **两条软约束改「聪明」不改「严苛」**——「叙事数字零虚构 vs 建议句惯用语豁免」「名词小抄以『跟不上主线』为线而非逐词全解释」，避免评分口径漂移误伤惯用语。
...
v1.1 变更：
- **名词小抄排版规则反转**：v1.0 的「用引用块集中解释」实测在小站渲染器翻车（引用符与列表拆开显示），改为「加粗标题 + 普通无序列表」，并明令禁止引用块内嵌列表；
- **全角标点从体例要求升级为程序化校验流程**：手写/生成保证不了全角（实测一篇 47 处漏网），必须成稿后跑扫描-转换-复检到 0 残留；转换护住代码围栏与行内代码（要被复制执行的内容保持半角），脚本用 Unicode 码点指定全角字符（生成端字面字符会半/全角混淆）；
- **新增第 3 步「发布后回收终稿」**：作者总会在平台编辑器做最后微调（拆段/删 `---`），发布后抓线上正文回写库内母版，只回写内容与分段、不动 Markdown 格式，保持「库内版 = 线上版」；
- 脱敏补例外（公开复盘自认作品时项目名与公开平台 ID 可保留）；补「段落宜短」「可抄资产是传播点」两条软经验。

### feishu-doc-publish v1.0（2026-07-12，真机发布验证封装）
...
### opportunity-due-diligence v1.0（2026-07-31，Melolo 拉新招募实战封装）

来源：学员转来「Melolo（字节旗下印尼短剧 APP）在 TikTok 拉新剪辑」招募，做完调研后沉淀。

- [[opportunity-due-diligence/SKILL.md]] — Claude
- 产出样例：`08_对外分发/海外短剧剪辑拉新副业能不能做_尽调笔记.md`（+ 同名 `.png` 长图）

定位：把「这个副业靠谱吗」这类提问，收敛成可重复的尽调流程。编码的关键判断——① **「是不是骗局」和「值不值得做」必须分开下结论**，最常见的真实答案是第三象限「不是骗局但极不划算」，把两问揉成一个就只能得到「小心有诈」这种没用的回答；② **前置收费是骗局与低价外包的分水岭**（课程费/授权费/押金/办卡/提现门槛），二值判据，效率极高，且央视曝光的短剧推广骗局核心特征就是它；③ **最决定性的是第 4 步「能否验证结算依据」**——计价单位定义空着的那个词最值钱，「账号与发布由项目方统一处理」这类话术等于无对账能力 + 影响不了转化结果 + 零资产沉淀；④ 时薪必须自己按乐观口径算完整链条（对方只给单价从不给时薪）；⑤ 必问清单里放「上一批参与者收入的**中位数**」，只肯给个案最高值的直接判负。体例上确立**称谓律**：直接转发给当事人读的稿子一律第二人称，文件名不带 `_学员版`，分发层级只写在索引里。

### report-longimage v1.0（2026-07-31，PB Arena 报告版式产线化）

来源：同一会话把上述尽调笔记做成「pb-arena 同款图片」，把版式与出图流程固化。

- [[report-longimage/SKILL.md]] — Claude
- 随附资产：`母版.html`（组件画廊）· `render.sh`（两趟截图脚本）

定位：把 PB Arena 更新报告长图的版式变成任何文档都能套的产线。编码的关键经验——① **出图必须两趟**：`--screenshot` 只截 `--window-size` 那么大的视口，先 `--dump-dom` 读页面自报的 `scrollHeight` 再按精确高度截第二趟（母版末尾那行 `document.title = "H" + scrollHeight` 是读数口，删了脚本就跑不了）；② **`--mono` 以 generic `monospace` 收尾会让中文全掉进宋体**——`monospace` 通配一切，排它后面的字体永不生效，CJK 必须排在它**之前**，这条是 [[导出运行时无系统字体回退律_CJK豆腐块_v1]] 的又一次应用；③ `set -e` 下用 `&&` 链找文件会静默退出（循环体最后一条返回非 0 即终止且不报错），要写 `if…fi`；④ **验收必须裁头/中/尾三段放大回看**，整图缩略图看不出字体回退和漏改的文案——本次署名写错（跳猪 → 跳蛛）与 CJK 掉宋体两个问题都是靠裁图才发现的。视觉规范出处见 [[美工改稿全站落地_跨稿重复才是规范_v1]]。

### dingtalk-doc-delivery v1.0（2026-07-17，实战6附件版一轮过审封装）

来源：《实战6-自动化口播剪辑》附件版并入公司钉钉知识库母文档的完整交付——API 推送卡审批改走手动导入 → 单日 4 个格式变体试探导入通道 → 母文档钉钉侧人工修正过审 → 终版下载 verbatim 回填 git → diff 提炼出格式白名单，全程走通后封装。

- [[dingtalk-doc-delivery/SKILL.md]] — Claude
