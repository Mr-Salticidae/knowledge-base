...
- 滤掉宽高比 < 0.6 的竖图（裁到 16:10 会废）、按内容 MD5 去重。
- **素材就是 640×853，提不了**。尺寸由直链末尾的 CDN 预设（`!nc_n_webp_mw_1`）决定，
  `imageView2` 参数被忽略，换预设后缀直接 403（签名绑死）。够用——文字在 1920px 画布上
  矢量绘制再降采样，锐度不受影响；背景略软反而贴「真实手机照」的调性。
