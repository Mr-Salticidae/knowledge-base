...
| [insight-public-post](#insight-public-post) | 对外分发 | 知识库内核档 → 公开版对外帖(B 站 AI 开发者小站),发布后回收终稿 | Claude |
...
**触发词**：「发小站」「写公开版」「同步对外帖」「把这篇沉淀分发出去」「写成帖子」「产出一份发布在AI开发者小站的帖子」——只要涉及「知识库内容 → 对外公开发布」即触发，即使没提「小站」二字。

**用途**：把知识库内核档（含双链、面向自己）改写成陌生人可冷读的公开帖，B 站 AI 开发者小站首发。三步半：① 定位内核档 + 读黄金样例；② 按 08 分发体例（零双链 / 名词小抄用加粗标题+普通列表 / 全角标点程序化校验）+ B 站人设（整活自黑 / 事故现场钩子 / 结尾可抄清单）+ 三层脱敏改写；③ 归档 08_对外分发 + 索引登记 + 提交推送；④ **发布后回收终稿**——抓线上正文回写库内母版，保持库内版 = 线上版。
...
**关联方法论**：[[07_skill存档索引]] · 黄金样例 `08_对外分发/AI日报全绿却停更_云端定时任务连环翻车复盘_公开版.md`
...
**触发词**：「这个副业靠谱吗」「帮我看看这个项目」「有人发来一个招募」「这个能做吗」「是不是骗局」「帮我调研一下这个机会」——转来一份招募文案 / 海报要求判断即触发。

**用途**：对「低门槛赚钱机会」做尽职调查（副业招募、内容代剪外包、代理分销、拉新返佣、地推），产出可直接转发给当事人的尽调笔记。

**核心心法**：**「是不是骗局」和「值不值得做」是两个独立结论，必须分开下。** 最常见的真实答案是第三象限——不是骗局，但极不划算。

**四步查证法（顺序不能换）**：① 查甲方是否存在（查不到直接出局）→ ② 查行业对标价 + 画转包链标出当事人在第几格 → ③ 查是否前置收费（**骗局与低价外包的分水岭，二值判据**）→ ④ **查能否验证结算依据**（最易忽略、最决定性：计价单位定义空着的那个词最值钱；「统一处理」类话术 = 无对账能力 + 影响不了结果 + 零资产沉淀）。

**三个附加动作**：自己按乐观口径把时薪算一遍（对方只给单价从不给时薪）；把宣传口径与免责口径的原话并排放（落差本身是证据）；必问清单里放「上一批参与者收入的**中位数**」。

**体例硬规则**：正文一律第二人称（直接转发给当事人读，不能把读者写成第三方，文件名也不带 `_学员版`）；不替人做决定，给可执行判据而非「建议别去」；必须给替代路径与真实的正面项；用「两周止损实验」代替「别做」。

**不适用**：正式 offer 薪资谈判；投资/理财/加密标的（不提供个性化投资建议）；明显涉刑活动（直接指出性质，不做划算分析）。
...
**触发词**：「做成 pb-arena 同款图片」「出一张长图」「把这篇做成报告长图」「渲染成图发群」「做张图发朋友圈」——任何「文档 → 可直接转发的长图」的诉求即触发，即使没提 pb-arena。
...
**硬规则**：珊瑚红 `#ff5a52` 占屏 5%-8%，`.hl` 全页 ≤8 处、`.redline` 整篇限一次；无渐变/光晕/厚阴影/玻璃拟态，层级只靠字重、对齐、留白、细线；指标栏固定四格且注解行要放**对照值**；署名固定「跳蛛先生」。

**三个已知坑**：① `--screenshot` 只截视口 → 必须两趟（先 `--dump-dom` 读 `scrollHeight`，母版末尾那行 script 是读数口，删不得）；② `--mono` 以 generic `monospace` 收尾会让中文全掉进宋体 —— **CJK 字体必须排在 `monospace` 之前**；③ `set -e` 下用 `&&` 链找文件会静默退出，要写 `if…fi`。

**验收纪律**：渲染完**必须裁头/中/尾三段放大回看**，缩略图看不出字体回退和漏改的文案（2026-07-31 那次署名写错与 CJK 掉宋体都是靠裁图才发现的）。
...
**触发词**：「给我出 20 个 AI 爆款主题」「爬小红书图片做封面」「批量生成视频封面」「选题+封面一起搞」「做一批大字封面」

**用途**：一条流水线跑完「产选题 → 采集背景素材 → 合成封面矩阵 → 拼审图大图」。产出 N 条选题 × M 张背景的 960×600（16:10）大字封面，供 A/B 挑选。选题由模型按 10 个钩子公式产出，背景走 Playwright 带登录态从小红书取真实生活场景照，合成走 Pillow 参数化排版。
