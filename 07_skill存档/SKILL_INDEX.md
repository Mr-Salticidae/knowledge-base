...
| [creator-weekly-report](#creator-weekly-report) | 创作者周报 | 一周活动(git+复盘+skill+发布)→ 事实先行结构化周报 | Claude |
...
## creator-weekly-report

**触发词**：「写周报」「这周做了什么」「本周总结 / 周复盘 / 周记」「生成创作周报」「把这一周沉淀一下」「做一份对外 / 发社群的周报」

**用途**：把一周散落在 git 提交、新增/改动的复盘与方法论、新建/升级的 skill、库外发布物与平台数据里的活动，聚合成一份**事实先行、结构化、能看出趋势**的周报。是日频复盘之上的「周」聚合层。五阶段：取数（自动抓 git 等客观信号）→ 事实冻结 → 写周报 → 归档自用内核版（`00_仓库维护/周报/`）→ 可选对外版（`08_对外分发/`）。

**版本状态**：v1.0 · 2026-06-29 · 从「日频复盘缺周聚合层」缺口封装；首期实例 [[2026-W26_创作周报]] 实跑自验证。

**文件路径**：`E:\knowledge-base\07_skill存档\creator-weekly-report\SKILL.md`

**与相邻 skill 的区别**：`aigc-postmortem` 是单作品/单比赛的纵向深挖；`knowledge-base-curator` 是一次答疑成果的沉淀与对外；本 skill 做**一周 N 个产出的横向聚合**（节奏 / 连续性 / 跨周趋势），借用前者的事实先行与后者的内核/对外双产出，不替代单点深挖。

**周报序列**：[[周报索引]]（`00_仓库维护/周报/`）
