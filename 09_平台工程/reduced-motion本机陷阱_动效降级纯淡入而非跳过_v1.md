...
- ⚠️ [[赛事状态机到点不切换_写入方不等于推进方_v1]] —— 同族「环境差异导致只有部分人复现」的镜像案例:本篇是本机偏好开关让作者自己看不到效果,那篇是用户侧最小字号/缩放先撞到排版临界点(其余人其实也已在临界点内侧);共同解法都是先量数值找临界点,别先归因于"那台机器的问题"
- [[任务系统演进_站内新建管理台拆分首页瘦身_v1]] —— **本轮复用**:滚动渐现从四页各写一份抽成共用 `revealOnScroll()` 模块,reduced-motion 纯淡入降级策略沿用本篇;刊头 `.masthead*` 三页重复副本也顺手收进全局
- [[09_平台工程索引]] —— 平台工程区入口
