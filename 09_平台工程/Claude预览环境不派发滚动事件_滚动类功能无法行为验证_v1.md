...
- [[reduced-motion本机陷阱_动效降级纯淡入而非跳过_v1]] —— 同族再验(2026-07-24):headless Chrome 上报的是本机 OS 的 reduce 偏好,不加 `--blink-settings=prefersReducedMotion=false` 测到的是降级路径;附 virtual-time-budget 下"已生效并清理完毕"被误判"没生效"的口径坑
- ⚠️ [[美工改稿全站落地_跨稿重复才是规范_v1]] —— 同族「够不着的 UI 退到什么」再验(2026-07-26):那次是运营后台要登录、本机又没 Node 起不了服务,退到**往容器注入样例 DOM + `getComputedStyle` 量栅格与计算色值**;新增的纪律是交付时必须把验证等级如实写出来(「这是量出来的,不是看到的」),并把需要人工点一遍的部分明确交回作者
