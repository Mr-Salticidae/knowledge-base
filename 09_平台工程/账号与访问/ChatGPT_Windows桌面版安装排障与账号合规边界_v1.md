...
- ⚠️ [[ping通不等于路通_fake-ip假信号与节点带宽实测选型_v1]] —— 同目录、同一台机器的 Clash 环境，是本文第三层「把网络检测当证据，不当占卜」的展开：本文记下 fake-ip 会让 DNS 返回 `198.18.0.0/15` 保留网段（内部映射，不等于泄露），那篇补上它的另一面——**这同时让 ping / tracert 等一切基于 IP 的连通性测试失效**，且这类假信号永远是绿的，最容易被当成健康证明
- ⭐ [[信任资产保得住造不出_账号基线vs人群先验_v1]] —— 同域互补:本篇校正「403=风控」「改地区/时区即可防封号」等**过度推断**(否定侧),那篇给出正面解释——真正起作用的是账号基线与人群先验的差别,而那一层任何脚本都碰不到
- [[炉石传说安装被拒_UBR判断假正常系统与Win10原地升级_v1]] —— 同为 Windows 桌面软件「装不上」的排障,但根因在**另一层**:本篇纠偏的是账号/网络链路层的过度归因,那篇补上「系统版本资格」层——版本号看着正常但 UBR 异常低(假正常系统),最低系统要求判定会静默拒绝安装
