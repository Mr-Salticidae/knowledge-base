...
### 第八次验证（2026-09-17 · vpn-guard 目录整理）⚠️ —— 命令里写的反斜杠，到 shell 手里少了一层

**形态八：agent 在命令里写的 `\\`，bash 收到时只剩一个 `\`；再加上 Git Bash 的 `sed -i` 会改写换行符，而 `git diff` 显示不出换行差异。**

当时要用 `sed -i` 把 `"$PSScriptRoot\browse-vpn.ps1"` 批量改成 `"$PSScriptRoot\..\browse-vpn.ps1"`，命令里按常规写了双反斜杠。结果三处同时出错，而且都没有报错：

- **模式没匹配上**：`\\b` 到 sed 手里变成 `\b`，被当成词边界，7 个 `.ps1` 一个都没改到。sed 匹配不到时不报错；
- **替换写进了控制字符**：`.cmd` 里的 `..\\app` 变成 `..\app`，其中 `\a` 被 sed 解释成响铃符（0x07），终端里看不出来；
- **换行符被改写**：经 `sed -i` 处理的文件，CRLF 全部变成了 LF。仓库开着 `core.autocrlf=true`，所以 `git diff` 不显示换行差异，只给了一句「LF will be replaced by CRLF」的警告。

露馅的信号有三个：`git diff --stat` 只列出 1 个文件（应该是 8 个）；`cat -A` 显示出 `^G`；`file` 显示换行符变了。

事后定位：bash 的单引号本应原样输出，但 `printf '%s\n' 'a\\b'` 打印出的是 `a\b`。可见**反斜杠在命令交给 bash 之前就少了一层**，问题不在 sed，也不在 MSYS（见 [[2026-09-17_vpn-guard目录整理收口_挪脚本等于挪状态与四处静默放行_迭代复盘_v1]]）。

**解法**：带反斜杠的 Windows 路径，不要用 shell 命令去改，改用文件编辑工具。它按字面写入，本次实测也保留了原文件的 BOM 和 CRLF。改完之后用 `file` 核对编码和换行，再用 `git diff --stat` 核对改动的文件数是否符合预期。

> 和形态六是同一句话：**调用方写下的，不等于被调方收到的。** 形态六是路径被转换层改写，形态八是转义在更早的一层就被吃掉了。
> 另外，`git diff` 在 autocrlf 下不报换行差异，这是又一层「所见」：它展示的是规范化之后的比较结果，不是工作区文件的实际字节。
...
5. **改完脚本文件**：用 `file` 看 BOM 和换行符，用 `git diff --stat` 数改动的文件数。`core.autocrlf=true` 时，`git diff` 不会显示换行差异。
...
- [[2026-09-17_vpn-guard目录整理收口_挪脚本等于挪状态与四处静默放行_迭代复盘_v1]] —— 形态八(第八次验证)来源(2026-09-17):命令里的 `\\` 到 bash 只剩一个,sed 静默匹配不到、`\a` 被写成响铃符、CRLF 被改写,而 `git diff` 不显示
