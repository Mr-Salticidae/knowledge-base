...
| 改要"透明背景"省掉抠像,结果拿到**画上去的灰白棋盘格**(假透明) | 这是**第三方代跑平台**丢了 alpha 通道,不是模型不会——换原厂(GPT 官网)出即得真 RGBA;不换平台就老实走绿幕。见 [[标签相同不等于行为相同_第三方代跑平台律_v1]] |
...
- ⚠️ [[标签相同不等于行为相同_第三方代跑平台律_v1]] —— 本法为什么用**绿幕**而不是直接要透明底:代跑平台可能给不出 alpha,只画一层灰白棋盘格糊弄过去(《Last Stand》怪物实际踩过,卡了一个版本)
