...
- 症状像、根因差一层:[[标签相同不等于行为相同_第三方代跑平台律_v1]] —— 本篇是**模型渲染**毁抠图(玻璃高光+投影,物理正确所以躲不掉);那条是**平台管线**根本没给 alpha 通道,模型只好画一层灰白棋盘格当"透明"。拿到"抠不干净"的图先分清是哪一层
- 主线交接:`{AIGC工作站}/跨会话协作\B站直播弹幕插件_全局进度_交接给_下一次Cowork.md` §三 应用图标决策
- 实证素材:`{项目工作区}/B站直播弹幕+礼物插件/resources\_processed\` 含本项目 v1-v6 的所有失败迭代(可对照学习)
