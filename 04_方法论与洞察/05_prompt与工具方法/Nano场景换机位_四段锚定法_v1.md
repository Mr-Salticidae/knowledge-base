...
> 关联:[[nano跨场景三变量发现_v1]] / [[prompt的三段式结构_v1]] / [[概念锚定_风格置换迭代法_v1]]
...
**为什么拆成「锚定段 + 机位描述」:** 把固定部分(1/2/4)抽成可复用的锚定段,机位(3)单列。试多个机位时基准一致、变量单一,便于对比择优。与 [[prompt的三段式结构_v1]] 同源——都是"把固定结构沉淀、把变量前置"的思路。
