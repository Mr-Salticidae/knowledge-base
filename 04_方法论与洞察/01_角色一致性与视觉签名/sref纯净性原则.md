...
## 十一之二、扩展判据（2026-08-26 补）：挂不挂 sref,看主体的**媒介**,不看画面里有没有那个元素

上一节的判据是"锁风格还是锁主体"。实践里还有第三种情况原文没覆盖——
**一张图里同时存在两种媒介**。

《擦干净》后续的"融合底图 v2"就是:**整张是写实纪录照片,只有一件小物件(茶缸 / 球鞋 / 咖啡杯)是 Taniguchi 线条画。**

当时的纠结:项目红线写着"线条画类必须带 `--sref` 锚点 + `--sw 100`",那这张算不算线条画类?
prompt 清单末尾的锚点表挂着"待填",一挂好几天,出图卡在这里。

**按本篇的机制推,答案是不挂**:

咖啡杯锚点是"纯线条 + 大留白"的纯净锚点。挂上去,sref 说"整张是线条画",prompt 说"整张是照片、只有杯子是线条"——
这正是本篇第三节描述的那个矛盾结构。MJ 的解决方式是**放弃权重最弱的部分**,
轻则线条质感丢失,重则把整张写实底图拖成线稿。

判据由此再推一层:

> **决定挂不挂 sref,看的是画面主体的媒介,不是画面里有没有那个风格的元素。**
> 主体是照片 → 走写实组规格,不挂线条锚点,那一件物件靠 prompt 内描述;
> 主体是线条画 → 挂锚点。
> "画面里有线条画元素"**不构成**挂线条锚点的理由。

同批有一张是真该挂的:S26 的"线条勾边卧室"——整个房间每个物体都被墨线勾边、颜色在褪,
线条覆盖全画面、是画面的主导媒介。这张挂锚点,并配了 `--sw 100 → 50 → 去掉` 的阶梯兜底
(防它一路拖成纯线稿,把照片底层吃掉)。

⚠️ **验证状态:推论,不是实证**——本次没跑挂 sref 的 A/B 对照组。
若出图后发现不挂 sref 的物件线条与素材库脱节,这条要回来改。
...
- [[单锚点优于三视图_v1]] —— 同源结论的另一个面：参考的纯净性比数量重要，40 镜实跑数据
- [[2026-08-26_擦干净_剧本口径审计与补图承接位_复盘_v1]] —— 十一之二「看主体媒介」判据的来源；同案还给出一个**允许**的沿用法：同主体、同取景、只换光线时，可拿本组第一张定稿当 sref 跑后续
- [[骨架篇幅即权重_v1]] —— 张力方向：不挂 sref 时，靠长骨架锁取景（但明确不适用于锁脸）
- [[MJ_v8.2_行为档案_v1]] —— 第 3 条「sref 跨批质感续接」的实测依据
